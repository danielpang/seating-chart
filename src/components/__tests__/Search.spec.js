...
const testCases = {
  standardMatches: [
    { input: 'joh', target: 'John Smith', expectedResult: true, description: 'start of first name' },
    { input: 'smith', target: 'John Smith', expectedResult: true, description: 'full last name' },
    { input: 'ohn', target: 'John Smith', expectedResult: true, description: 'middle of name' }
  ],
  
  noMatches: [
    { input: 'jsm', target: 'John Smith', expectedResult: false, description: 'non-consecutive letters' },
    { input: 'jhn', target: 'John Smith', expectedResult: false, description: 'missing letter' },
    { input: 'xyz', target: 'John Smith', expectedResult: false, description: 'no matching letters' }
  ],
  
  caseVariations: [
    { input: 'SMITH', target: 'John Smith', expectedResult: true, description: 'uppercase input' },
    { input: 'smith', target: 'John SMITH', expectedResult: true, description: 'uppercase target' },
    { input: 'SMiTh', target: 'john smith', expectedResult: true, description: 'mixed case' }
  ],
  
  specialCases: [
    { input: '', target: 'John Smith', expectedResult: false, description: 'empty input' },
    { input: ' ', target: 'John Smith', expectedResult: false, description: 'space only' },
    { input: "o'b", target: "John O'Brien", expectedResult: true, description: 'apostrophe' },
    { input: '-sm', target: 'John-Smith', expectedResult: true, description: 'hyphenated name' }
  ],
  
  multipleMatches: [
    { input: 'an', target: 'Anna Hannah', expectedResult: true, description: 'multiple possible matches' },
    { input: 'han', target: 'Anna Hannah', expectedResult: true, description: 'match in second word' }
  ]
};
    ...
    it('runs all CSV test cases', () => {
      csvTestCases.forEach(({ input, target, expectedResult, description }) => {
        ...
        if (result !== expectedResult) {
          console.log(`Tested: ${description} - ${result === expectedResult ? 'PASS' : 'FAIL'}`);
        }
      });
    });
  });
